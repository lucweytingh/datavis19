...
market = pd_data.filter({ 'market_name': 'Ntunga', 'item_name': ['Cassava leaves', 'Cassava', 'Cassava flour' ]}).dict_from_columns(['market_name', 'item_name'], ['price_usd', 'date'])
...
plot1 = figure(x_axis_type="datetime", title='Prijs van Cassava flour en Cassava leaves in Ntunga, Rwanda', sizing_mode='stretch_both')
legend = []
# legend.append(('Aantal',[add_to_plot(plot1,'rowcount',dates,rowcount)]))

for item, data in market['Ntunga'].items():
