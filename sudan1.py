...
market = pd_data.filter({ 'country_name': 'South Sudan', 'market_name': 'Jau' }).dict_from_columns(['market_name', 'item_name'], ['price', 'date'])
...
plot1 = figure(x_axis_type="datetime", title='Productprijzen in Jau, Zuid Soedan', sizing_mode='stretch_both')
legend = []
# legend.append(('Aantal',[add_to_plot(plot1,'rowcount',dates,rowcount)]))

for item, data in market['Jau'].items():
